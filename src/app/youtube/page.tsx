  ...
  return <div className="space-y-7 pb-6">
    ...
    {state==='results'&&analysis&&<>
      ...
      <section className="card p-5 md:p-6 space-y-5"><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4"><div><p className="muted text-sm">Found</p><h2 className="text-2xl font-black">{analysis.vocabulary.length} useful words <span className="muted text-base font-medium">· {knownCount} already known · {analysis.vocabulary.length-knownCount} new</span></h2></div><div className="flex gap-3"><button className="btn secondary" disabled={!selectedItems.length} onClick={()=>addItems(selectedItems)}><Plus size={18}/>Add selected ({selectedItems.length})</button><button className="btn primary" disabled={!analysis.vocabulary.some(item=>!knownByWord.has(wordKey(item.word)))} onClick={()=>addItems(analysis.vocabulary.filter(item=>!ignoredIds.includes(item.id)))}><Check size={18}/>Add all</button></div></div>
        <div className="pt-5 border-t" style={{borderColor:'var(--line)'}}><div className="flex items-center gap-2 font-bold mb-3"><Filter size={18}/>Filters</div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3"><label className="btn justify-start"><input className="w-5 h-5" type="checkbox" checked={allVisibleSelected} onChange={event=>setSelectedIds(ids=>event.target.checked?[...new Set([...ids,...selectableVisible.map(item=>item.id)])]:ids.filter(id=>!selectableVisible.some(item=>item.id===id)))}/>Select all visible</label><label className="btn justify-start"><input className="w-5 h-5" type="checkbox" checked={onlyUnknown} onChange={event=>setOnlyUnknown(event.target.checked)}/>Only unknown words</label><label className="btn justify-start"><input className="w-5 h-5" type="checkbox" checked={onlyB2Plus} onChange={event=>setOnlyB2Plus(event.target.checked)}/>Only B2+</label><label className="btn justify-start"><input className="w-5 h-5" type="checkbox" checked={partFilter==='Verb'} onChange={event=>setPartFilter(event.target.checked?'Verb':'all')}/>Only verbs</label><label className="btn justify-start"><input className="w-5 h-5" type="checkbox" checked={partFilter==='Noun'} onChange={event=>setPartFilter(event.target.checked?'Noun':'all')}/>Only nouns</label><label className="btn justify-start"><input className="w-5 h-5" type="checkbox" checked={partFilter==='Adjective'} onChange={event=>setPartFilter(event.target.checked?'Adjective':'all')}/>Only adjectives</label></div><label className="block mt-4 max-w-sm"><span className="text-sm font-bold">Show vocabulary from level</span><div className="relative mt-2"><select value={minimumLevel} onChange={event=>setMinimumLevel(event.target.value as CefrLevel)}>{CEFR_LEVELS.map(level=><option value={level} key={level}>{level}{level===nextCefrLevel(store.settings.learnerLevel)?' · recommended for you':''}</option>)}</select><ChevronDown className="absolute right-4 top-4 pointer-events-none muted" size={17}/></div></label></div>
      </section>
