...
function Game({target,guess,setGuess,guesses,hintsUsed,useHint,error,loading,submit,feedback,proximityForRank}:{target:WordHuntTarget;guess:string;setGuess:(v:string)=>void;guesses:SemanticGuess[];hintsUsed:number;useHint:()=>void;error:string;loading:boolean;submit:()=>void;feedback:string;proximityForRank:(rank:number)=>Proximity}){
  ...
  return <View className="gap-5">
    <View className="items-center pt-3 gap-2"><View className="w-14 h-14 rounded-[20px] bg-mint items-center justify-center"><Target size={28} color={colors.green}/></View><Text className="text-green text-xs font-black tracking-[2px] mt-2">{pick('НАЙДИТЕ СЛОВО','FIND THE WORD')}</Text><Text className="text-[30px] leading-[36px] font-black text-ink text-center">{pick('Я загадал английское слово','I’m thinking of an English word')}</Text><Text className="text-muted text-center leading-6 max-w-[420px]">{pick('Введите любое слово и узнайте, насколько оно близко по смыслу.','Type any word and see how close it is in meaning.')}</Text></View>
